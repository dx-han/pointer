//! Eventually consistent server-side logic for Pointer.
...
use crate::{model::PersistedDocument, ot::transform_index};
...
impl From<PersistedDocument> for Pointer {
    fn from(document: PersistedDocument) -> Self {
        let mut operation = OperationSeq::default();
        operation.insert(&document.text);

        let pointer = Self::default();
        {
            let mut state = pointer.state.write();
...
impl Pointer {
    ...
    /// Returns if this Pointer object has been killed.
