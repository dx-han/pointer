//! Backend SQLite data model handlers for persisting documents.
...
/// Represents a document persisted in data model storage.
...
/// A driver for data model operations wrapping a pool connection.
#[derive(Clone, Debug)]
pub struct Model {
    pool: SqlitePool,
}
...
impl Model {
    /// Construct a new data model from Postgres connection URI.
    pub async fn new(uri: &str) -> Result<Self> {
        {
            // Create data model file if missing, and run migrations.
    ...
    /// Load the text of a document from the data model.
    ...
    /// Store the text of a document in the data model.
...
ON CONFLICT(id) DO UPDATE SET
    ...
    /// Count the number of documents in the data model.
