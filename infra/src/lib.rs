//! Server backend for the Pointer collaborative text editor.
...
use crate::{model::Model, pointer::Pointer};
...
impl Document {
    fn new(pointer: Arc<Pointer>) -> Self {
        Self {
...
struct ServerState {
    ...
    /// Connection to the data model pool, if persistence is enabled.
    model: Option<Model>,
}
...
struct Stats {
    ...
    num_documents: usize,
    /// Number of documents persisted in the data model.
    model_size: usize,
}
...
pub struct ServerConfig {
    ...
    pub expiry_days: u32,
    /// Data model object, for persistence if desired.
...
async fn socket_handler(id: String, ws: Ws, state: ServerState) -> Result<impl Reply, Rejection> {
    ...
    let mut entry = match state.documents.entry(id.clone()) {
        ...
        Entry::Vacant(e) => {
            let pointer = Arc::new(match &state.model {
                Some(db) => db.load(&id).await.map(Pointer::from).unwrap_or_default(),
                None => Pointer::default(),
            });
            if let Some(db) = &state.model {
                tokio::spawn(persister(id, Arc::clone(&pointer), db.clone()));
            }
            e.insert(Document::new(pointer))
        }
    };

    let value = entry.value_mut();
    value.last_accessed = Instant::now();
    let pointer = Arc::clone(&value.pointer);
    Ok(ws.on_upgrade(|socket| async move { pointer.on_connection(socket).await }))
}
...
async fn stats_handler(start_time: u64, state: ServerState) -> Result<impl Reply, Rejection> {
    ...
    let model_size = match state.model {
        None => 0,
...
async fn persister(id: String, pointer: Arc<Pointer>, db: Model) {
    let mut last_revision = 0;
    while !pointer.killed() {
        ...
        if revision > last_revision {
            ...
            if let Err(e) = db.store(&id, &pointer.snapshot()).await {
